...
for tour=1:L
    if (tour==1)
        ...
        for i=1:2
            ...
            res1 = Metrique(etat,tour) + sortieb*section;
            %Calcul de la métrique
            if( ((tour <= K)) || (~closed) )
                next_state = treillis.nextStates(etat,i);
                Metrique(next_state+1,tour+1) = min(Metrique(next_state+1,tour), res1);
        ...
        for etat=1:numStates
            for i=1:2
                ...
                % Procédure en cas de fermeture
                    ...
                    % calcul du poids
                    Metrique(next_state+1,section+1) = min(Metrique(next_state+1,tour), res1);
