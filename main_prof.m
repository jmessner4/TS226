...
N = K*2; % Nombre de bits codés par trame (codée) %Chercher la vrai valeur
...
figure(1)
semilogy(EbN0dB,Pb_u,'--', 'LineWidth',1.5,'DisplayName','Pb (BPSK théorique) (non codé)');
hold all
semilogy(EbN0dB,Pe_u,'--', 'LineWidth',1.5,'DisplayName','Pe (BPSK théorique)(non codé)');
...
for iSNR = 1:length(EbN0dB)
    ...
    while (bitErr < nbrErreur && pqtNbr*K < nbrBitMax)
        ...
        treillis=poly2trellis(3,[5,7]);
            ...
            % A FAIRE POUR LES AUTRES TREILLIS !!

        s_i=0;
        closed=false;
        c       = cc_encode(u,treillis,s_i,closed); % Encodage
